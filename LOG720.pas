...
var
  FToolProvxArtDet: TFToolProvxArtDet;
...
procedure TFToolProvxArtDet.FormCreate(Sender: TObject);
begin
...
procedure TFToolProvxArtDet.Timer1Timer(Sender: TObject);
begin
   //FVariables.w_NombreForma := 'FToolProvxArt';
   if (DMLOG.wAdmin='G') or (DMLOG.wAdmin='P') then Exit;
   if not xAccesoBotones then
   begin
      DMLOG.AccesosUsuariosR( DMLOG.wModulo,DMLOG.wUsuario,'2',FToolProvxArtdet);
      xAccesoBotones := True;
   end;
   Timer1.Destroy;
end;

procedure TFToolProvxArtDet.dblcCiaExit(Sender: TObject);
begin
   edtCIA.text:=DMLOG.DisplayDescrip('prvSQL','TGE101','CIADES','CIAID='+quotedStr(dblcCIA.text),'CIADES');
end;

procedure TFToolProvxArtDet.bbtnRep2Click(Sender: TObject);
var
	xWhere, xOrder, xSQL, FRegDes, FRegHas : String;
  x, x10 : Integer;
begin

  xSQL:=Mtx5001.FMant.SQL;
  ...
  ppR1.TEMPLATE.FileName := ExtractFilePath( application.ExeName ) + wRutaRpt+'\ComprasxProveedorDet.rtm' ;
...
procedure TFToolProvxArtDet.sbDisenoRepClick(Sender: TObject);
begin
