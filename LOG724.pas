...
var
  FToolArtxProvOC: TFToolArtxProvOC;
...
procedure TFToolArtxProvOC.FormCreate(Sender: TObject);
begin
...
procedure TFToolArtxProvOC.Timer1Timer(Sender: TObject);
begin
   //FVariables.w_NombreForma := 'FToolProvxArt';
   if (DMLOG.wAdmin='G') or (DMLOG.wAdmin='P') then Exit;
   if not xAccesoBotones then
   begin
      DMLOG.AccesosUsuariosR( DMLOG.wModulo,DMLOG.wUsuario,'2',FToolArtxProvOC);
      xAccesoBotones := True;
   end;
   Timer1.Destroy;
end;

procedure TFToolArtxProvOC.dblcCiaExit(Sender: TObject);
begin
   edtCIA.text:=DMLOG.DisplayDescrip('prvSQL','TGE101','CIADES','CIAID='+quotedStr(dblcCIA.text),'CIADES');
end;

procedure TFToolArtxProvOC.bbtnRep2Click(Sender: TObject);
var
...
begin
  ...
  ppR1.TEMPLATE.FileName := ExtractFilePath( application.ExeName ) + wRutaRpt+'\ComprasxArticuloOC.rtm' ;
...
procedure TFToolArtxProvOC.sbDisenoRepClick(Sender: TObject);
begin
